...
def compute_inliers_nocalib(matched_kp1, matched_kp2, geom_model='H'):
    matched_pts1 = cv2.KeyPoint_convert(matched_kp1)
    matched_pts2 = cv2.KeyPoint_convert(matched_kp2)

    if 'H' in geom_model:
        # Estimate the homography between the matches using RANSAC
        M, inliers_M = cv2.findHomography(matched_pts1,
                                        matched_pts2,
                                        cv2.RANSAC,
                                        ransacReprojThreshold = 3.,
                                        confidence = 0.9999)
        if inliers_M is None:
            inliers_M = np.array([0])
    else:
        # Estimate the fundamental matrix between the matches using RANSAC
        M, inliers_M = cv2.findFundamentalMat(matched_pts1,
                                        matched_pts2,
                                        cv2.RANSAC,
                                        ransacReprojThreshold = 3.,
                                        confidence = 0.9999)
...
def preprocess_image(img_file, img_size, green=False):
    ...
    return img_preprocessed[31:543,102:614], img_orig
...
if __name__ == '__main__':
    ...
    filedir='/media/discoGordo/dataset_leon/hyper-kvasir/labeled-videos/lower-gi-tract/quality-of-mucosal-view/BBPS-2-3_frames'
    '''videos_seq = {}
    ...
    '''lvideos = [x[:-14] for x in os.listdir(filedir)]
    svideos = set(lvideos)
    videos = list(svideos)
    videos.sort()'''
    videos=range(0,31)
    ...
    f = open('/media/discoGordo/dataset_leon/colmap_MICCAI/colmap_benckmark_MICCAI_inliers_hyper-kvasir.txt','a+')
    fileouts='/media/discoGordo/dataset_leon/colmap_MICCAI/kps_hyper-kvasir'+"/"+prefe
    if not os.path.exists(fileouts):
        os.mkdir(fileouts)
    for v in videos:
        '''#if "00033" not in v:
        #    continue
        if not os.path.exists(fileouts+"/"+str(v)):
            os.mkdir(fileouts+"/"+str(v))
        err_qsdict[v]={}
        err_tsdict[v]={}
        for seq in videos_seq[v]:
            #if "30" != seq:
            #    continue'''
        print("Evaluating cluster:", "VIDEO "+str(v))#+" CLUSTER "+seq)
        if not os.path.exists(fileouts+"/"+str(v)):
            os.mkdir(fileouts+"/"+str(v))
        '''Ts = np.load("/media/discoGordo/dataset_leon/colmap_MICCAI/pseudoGT/"+v+"/"+seq+"/RandT.npy")
        RandT = open("/media/discoGordo/dataset_leon/colmap_MICCAI/pseudoGT/"+v+"/"+seq+"/RandT.txt", 'r')
        ...
        filename1 = 'hyper-kvasir'+'_'+str(v)+'_'+str(image_number1).zfill(5)
        img1_file=filedir+"/"+filename1+'.png'
        
        img1, img1_orig = preprocess_image(img1_file, img_size, green)
        img1_color=img1_orig[31:543,102:614]
        ...
        filelabel1=fileouts+"/"+str(v)+"/"+filename1+'.npz'
        if not spec and os.path.exists(filelabel1):
            ...
            for kp in kp1:
                #xsv.append(kp.pt[0])
                #xs.append(kp.pt[0]/530)
                #ysv.append(kp.pt[1])
                #ys.append(kp.pt[1]/530)
                x=kp.pt[0]
                #x=x+205
                y=kp.pt[1]
                kp.pt=(int(x),int(y))
        ...
        for i in range(0,maxframes-dist):
            image_number2=int(img2_number)+i
            ...
            #image_number1=int(img1_number)+i+1
            #continue
            '''if image_number1 > len(images_number):
                index_T1 = -1
            elif images_number[image_number1-1] == image_number1:
                index_T1 = image_number1-1
            elif images_number[image_number1-1] < image_number1:
                print("Usage error: gt=",images_number[image_number1-1]," image_number1=", image_number1)
                exit(1)
            else:
                index_previous = min(len(images_number),image_number1-1)
                while images_number[index_previous] > image_number1 and index_previous>0:
                    index_previous-=1
                if abs(image_number1-images_number[index_previous]) <= abs(image_number1-images_number[image_number1-1]):
                    index_T1 = index_previous
                else:
                    index_T1 = image_number1-1
            
            if image_number2 > len(images_number):
                index_T2 = -1
            elif images_number[image_number2-1] == image_number2:
                index_T2 = image_number2-1
            elif images_number[image_number2-1] < image_number2:
                print("Usage error: gt=",images_number[image_number2-1]," image_number2=", image_number2)
                exit(1)
            else:
                index_previous = min(len(images_number),image_number2-1)
                while images_number[index_previous] > image_number2 and index_previous>0:
                    index_previous-=1
                if abs(image_number2-images_number[index_previous]) <= abs(image_number2-images_number[image_number2-1]):
                    index_T2 = index_previous
                else:
                    index_T2 = image_number2-1'''
                    
            filename2 = 'hyper-kvasir'+'_'+str(v)+'_'+str(image_number2).zfill(5)
            img2_file=filedir+"/"+filename2+'.png'
            if not os.path.exists(img2_file):
                break
            
            img2, img2_orig = preprocess_image(img2_file, img_size, green)
            img2_color=img2_orig[31:543,102:614]
            ...
            filelabel2=fileouts+"/"+str(v)+"/"+filename2+'.npz'
            ...
            if len(kp1)>0 and len(kp2)>0:
                ...
                if len(matches)>0:
                    ...
                    #inliers_H, inliers_E, inliers_Ef, inliers_Ec, R_E, R_E2, t_E = compute_inliers_2(
                    #        m_kp1, m_kp2, 'E', img1_file, dR, dT)
                    if dist <= 5:
                        geom_model = 'H'
                    else:
                        geom_model = 'F'
                    inliers_H = compute_inliers_nocalib(m_kp1, m_kp2, geom_model)
                else:
                    inliers_H=[0]
                    '''#inliers_F=[0]
                    inliers_E=[0]
            ...
            for q in np.where(inliers_H==1)[0]:
                inliers_Hx=np.append(inliers_Hx,np.array([(m_kp1[q].pt[0])]))
                inliers_Hy=np.append(inliers_Hy,np.array([m_kp1[q].pt[1]]))
                inl_Hxs.append((m_kp1[q].pt[0])/512)
                inl_Hys.append(m_kp1[q].pt[1]/512)
            hist_H, xedges, yedges, quadmesh=plt.hist2d(inliers_Hx,inliers_Hy,bins=bins,range=[[0,512-1],[0,512-1]])
            if hist_Hv is None:
                hist_Hv=hist_H/(np.amax(hist_H)+1e-6)
            else:
                hist_Hv+=hist_H/(np.amax(hist_H)+1e-6)
                hists+=1
            ...
            inliers_Hy=np.array([])
            for q in np.where(inliers_H==1)[0]:
                inliers_Hx=np.append(inliers_Hx,np.array([(m_kp2[q].pt[0])]))
                inliers_Hy=np.append(inliers_Hy,np.array([m_kp2[q].pt[1]]))
                inl_Hxs.append((m_kp2[q].pt[0])/512)
                inl_Hys.append(m_kp2[q].pt[1]/512)
            hist_H, xedges, yedges, quadmesh=plt.hist2d(inliers_Hx,inliers_Hy,bins=bins,range=[[0,512-1],[0,512-1]])
            full_Hp.append(len(np.where(hist_H>0)[0])/(bins*bins))
            if hist_Hv is None:
                hist_Hv=hist_H/(np.amax(hist_H)+1e-6)
            ...
            if not np.amax(inliers_F)>1:
                for q in np.where(inliers_F==1)[0]:
                    inliers_Fx=np.append(inliers_Fx,np.array([(m_kp1[q].pt[0])]))
                    inliers_Fy=np.append(inliers_Fy,np.array([m_kp1[q].pt[1]]))
                    inlFxs.append((m_kp1[q].pt[0])/530)
                    inlFys.append(m_kp1[q].pt[1]/530)
            hist_F, xedges, yedges, quadmesh=plt.hist2d(inliers_Fx,inliers_Fy,bins=bins,range=[[0,530-1],[0,530-1]])
            fullFp.append(len(np.where(hist_F>0)[0])/(bins*bins))
            inliers_Fx=np.array([])
            inliers_Fy=np.array([])
            if not np.amax(inliers_F)>1:
                for q in np.where(inliers_F==1)[0]:
                    inliers_Fx=np.append(inliers_Fx,np.array([(m_kp2[q].pt[0])]))
                    inliers_Fy=np.append(inliers_Fy,np.array([m_kp2[q].pt[1]]))
                    inlFxs.append((m_kp2[q].pt[0])/530)
                    inlFys.append(m_kp2[q].pt[1]/530)
            hist_F, xedges, yedges, quadmesh=plt.hist2d(inliers_Fx,inliers_Fy,bins=bins,range=[[0,530-1],[0,530-1]])
            fullFp.append(len(np.where(hist_F>0)[0])/(bins*bins))'''
            '''inliers_Ex=np.array([])
            inliers_Ey=np.array([])
            for q in np.where(inliers_E==1)[0]:
                inliers_Ex=np.append(inliers_Ex,np.array([(m_kp1[q].pt[0])]))
                inliers_Ey=np.append(inliers_Ey,np.array([m_kp1[q].pt[1]]))
                inl_Exs.append((m_kp1[q].pt[0])/530)
                inl_Eys.append(m_kp1[q].pt[1]/530)
            hist_E, xedges, yedges, quadmesh=plt.hist2d(inliers_Ex,inliers_Ey,bins=bins,range=[[0,530-1],[0,530-1]])
            if hist_Ev is None:
                hist_Ev=hist_E/(np.amax(hist_E)+1e-6)
            else:
                hist_Ev+=hist_E/(np.amax(hist_E)+1e-6)
                hists+=1
            full_Ep.append(len(np.where(hist_E>0)[0])/(bins*bins))
            inliers_Ex=np.array([])
            inliers_Ey=np.array([])
            for q in np.where(inliers_E==1)[0]:
                inliers_Ex=np.append(inliers_Ex,np.array([(m_kp2[q].pt[0])]))
                inliers_Ey=np.append(inliers_Ey,np.array([m_kp2[q].pt[1]]))
                inl_Exs.append((m_kp2[q].pt[0])/530)
                inl_Eys.append(m_kp2[q].pt[1]/530)
            hist_E, xedges, yedges, quadmesh=plt.hist2d(inliers_Ex,inliers_Ey,bins=bins,range=[[0,530-1],[0,530-1]])
            full_Ep.append(len(np.where(hist_E>0)[0])/(bins*bins))
            if hist_Ev is None:
                hist_Ev=hist_E/(np.amax(hist_E)+1e-6)
            ...
            for q in np.where(inliers_Ef==1)[0]:
                inliers_Efx=np.append(inliers_Efx,np.array([(m_kp1[q].pt[0])]))
                inliers_Efy=np.append(inliers_Efy,np.array([m_kp1[q].pt[1]]))
                inl_Efxs.append((m_kp1[q].pt[0])/530)
                inl_Efys.append(m_kp1[q].pt[1]/530)
            hist_Ef, xedges, yedges, quadmesh=plt.hist2d(inliers_Efx,inliers_Efy,bins=bins,range=[[0,530-1],[0,530-1]])
            full_Efp.append(len(np.where(hist_Ef>0)[0])/(bins*bins))
            inliers_Efx=np.array([])
            inliers_Efy=np.array([])
            for q in np.where(inliers_Ef==1)[0]:
                inliers_Efx=np.append(inliers_Efx,np.array([(m_kp2[q].pt[0])]))
                inliers_Efy=np.append(inliers_Efy,np.array([m_kp2[q].pt[1]]))
                inl_Efxs.append((m_kp2[q].pt[0])/530)
                inl_Efys.append(m_kp2[q].pt[1]/530)
            hist_Ef, xedges, yedges, quadmesh=plt.hist2d(inliers_Efx,inliers_Efy,bins=bins,range=[[0,530-1],[0,530-1]])
            full_Efp.append(len(np.where(hist_Ef>0)[0])/(bins*bins))
            inliers_Ecx=np.array([])
            inliers_Ecy=np.array([])
            for q in np.where(inliers_Ec==1)[0]:
                inliers_Ecx=np.append(inliers_Ecx,np.array([(m_kp1[q].pt[0])]))
                inliers_Ecy=np.append(inliers_Ecy,np.array([m_kp1[q].pt[1]]))
                inl_Ecxs.append((m_kp1[q].pt[0])/530)
                inl_Ecys.append(m_kp1[q].pt[1]/530)
            hist_Ec, xedges, yedges, quadmesh=plt.hist2d(inliers_Ecx,inliers_Ecy,bins=bins,range=[[0,530-1],[0,530-1]])
            full_Ecp.append(len(np.where(hist_Ec>0)[0])/(bins*bins))
            inliers_Ecx=np.array([])
            inliers_Ecy=np.array([])
            for q in np.where(inliers_Ec==1)[0]:
                inliers_Ecx=np.append(inliers_Ecx,np.array([(m_kp2[q].pt[0])]))
                inliers_Ecy=np.append(inliers_Ecy,np.array([m_kp2[q].pt[1]]))
                inl_Ecxs.append((m_kp2[q].pt[0])/530)
                inl_Ecys.append(m_kp2[q].pt[1]/530)
            hist_Ec, xedges, yedges, quadmesh=plt.hist2d(inliers_Ecx,inliers_Ecy,bins=bins,range=[[0,530-1],[0,530-1]])
            full_Ecp.append(len(np.where(hist_Ec>0)[0])/(bins*bins))'''
        ...
        #cv2.imwrite('/media/discoGordo/dataset_leon/colmap_MICCAI/inlier_heatmaps_GT/'+prefe+'_'+v+'_'+img1_number+'_'+img2_number+'.png',(hist_Hv/hists)*255.)
    ...
    line='$'+format(np.mean(feat),".1f")+'$ & $'+format(np.mean(inl_H),".1f")+'/'+format(np.mean(matc),".1f")+'$ ($'+format(100.*np.mean(inl_H)/np.mean(matc),".1f")+'\%$) & $'+format(np.mean(np.array([np.std(inl_Hxs),np.std(inl_Hys)])),".3f")+'$ & $'+format(np.mean(full_Hp)*100,".1f")+'$'
    # & $'+format(np.mean(inl_E),".1f")+'/'+format(np.mean(matc),".1f")+'$ ($'+format(100.*np.mean(inl_E)/np.mean(matc),".1f")+'\%$) & $'+format(np.mean(np.array([np.std(inl_Exs),np.std(inl_Eys)])),".3f")+'$ & $'+format(np.mean(full_Ep)*100,".1f")+'$ & $'+format(np.mean(inl_Ef),".1f")+'/'+format(np.mean(matc),".1f")+'$ ($'+format(100.*np.mean(inl_Ef)/np.mean(matc),".1f")+'\%$) & $'+format(np.mean(np.array([np.std(inl_Efxs),np.std(inl_Efys)])),".3f")+'$ & $'+format(np.mean(full_Efp)*100,".1f")+'$ & $'+format(np.mean(inl_Ec),".1f")+'/'+format(np.mean(matc),".1f")+'$ ($'+format(100.*np.mean(inl_Ec)/np.mean(matc),".1f")+'\%$) & $'+format(np.mean(np.array([np.std(inl_Ecxs),np.std(inl_Ecys)])),".3f")+'$ & $'+format(np.mean(full_Ecp)*100,".1f")+'$    '+format(err_qs_mean,".3f")+" "+format(err_ts_mean,".3f")+" "+format(err_qs_std,".3f")+" "+format(err_ts_std,".3f")+" "+format(err_qs_max,".3f")+" "+format(err_ts_max,".3f")+" "+format(err_qs_min,".3f")+" "+format(err_ts_min,".3f")+" "+"\n"+str(below_th_q)+"\n"+str(below_th_t)
