...
def match_descriptors_cv2_BF(kp1, desc1, kp2, desc2, feature='sift', thsim = 0):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ratio', type=int, default=75,
                        ...
                        (default: 80)')
    parser.add_argument('--thsim', type=int, default=0,
                        help='Minimum similarity threshold for matching out of 100 \
                        (default: 80)')
    ...
    f = open('/media/discoGordo/dataset_leon/colmap_MICCAI/colmap_benckmark_MICCAI_inliers_E_saved.txt',"a+")
    ...
    for v in videos_seq:
        ...
        for seq in videos_seq[v]:
            ...
            if not os.path.exists(fileouts+"/"+str(v)+"/"+str(seq)):
                os.mkdir(fileouts+"/"+str(v)+"/"+str(seq))
            ...
            for i in range(index_T1,len(images_number)-1):
                ...
                if abs(images_number[index_T2] - image_number2) <= window:
                    ...
                    if len(kp1)>0 and len(kp2)>0:
                        if 'superpoint' in weights_name or 'sp_v6' in weights_name:
                            if ratio != 0.75:
                                m_kp1, m_kp2, matches = match_descriptors_cv2_ratio(kp1, desc1.astype('float32'), kp2, desc2.astype('float32'), weights_name, ratio)
                            else:
                                m_kp1, m_kp2, matches = match_descriptors_cv2_BF(kp1, desc1.astype('float32'), kp2, desc2.astype('float32'), weights_name, thsim)
            ...
            #cv2.imwrite('/media/discoGordo/dataset_leon/colmap_MICCAI/inlier_heatmaps_GT/'+prefe+'_'+v+'_'+seq+'_'+img1_number+'_'+img2_number+'.png',(hist_Ev/hists)*255.)

    if not spec and 'sift' not in weights_name:
        if ratio != 0.75:
            np.save('/media/discoGordo/dataset_leon/colmap_MICCAI/errors_public/6'+prefe+'_r'+str(ratio)+'_'+str(numberErrors)+'_'+img1_number+'_'+img2_number+'_'+str(window)+'.npy',err_qsdict)
            np.save('/media/discoGordo/dataset_leon/colmap_MICCAI/errors_t_public/6'+prefe+'_r'+str(ratio)+'_'+str(numberErrors)+'_'+img1_number+'_'+img2_number+'_'+str(window)+'.npy',err_tsdict)
        elif thsim != 0:
            np.save('/media/discoGordo/dataset_leon/colmap_MICCAI/errors_public/6' + prefe+'_th'+ thsim + '_' + str(
                numberErrors) + '_' + img1_number + '_' + img2_number + '_' + str(window) + '.npy', err_qsdict)
            np.save('/media/discoGordo/dataset_leon/colmap_MICCAI/errors_t_public/6' + prefe+'_th'+ thsim + '_' + str(
                numberErrors) + '_' + img1_number + '_' + img2_number + '_' + str(window) + '.npy', err_tsdict)
        else:
            np.save('/media/discoGordo/dataset_leon/colmap_MICCAI/errors_public/6' + prefe + '_' + str(
                numberErrors) + '_' + img1_number + '_' + img2_number + '_' + str(window) + '.npy', err_qsdict)
            np.save('/media/discoGordo/dataset_leon/colmap_MICCAI/errors_t_public/6' + prefe + '_' + str(
                numberErrors) + '_' + img1_number + '_' + img2_number + '_' + str(window) + '.npy', err_tsdict)
    elif not spec:
        np.save('/media/discoGordo/dataset_leon/colmap_MICCAI/errors_public/6'+prefe+'_r'+str(ratio)+'_'+str(numberErrors)+'_'+img1_number+'_'+img2_number+'_'+str(window)+'.npy',err_qsdict)
        np.save('/media/discoGordo/dataset_leon/colmap_MICCAI/errors_t_public/6'+prefe+'_r'+str(ratio)+'_'+str(numberErrors)+'_'+img1_number+'_'+img2_number+'_'+str(window)+'.npy',err_tsdict)
    print(numberGood, numberBad, numberErrors)
    #np.save("/media/discoGordo/dataset_leon/colmap_MICCAI/indexes/newE/distanceFtoGT.npy",distanceFtoGT)
    #np.save("/media/discoGordo/dataset_leon/colmap_MICCAI/indexes/newE/distanceFtoF.npy",distanceFtoF)
    #np.save("/media/discoGordo/dataset_leon/colmap_MICCAI/indexes/newE/distanceGTtoGT.npy",distanceGTtoGT)
    #np.save("/media/discoGordo/dataset_leon/colmap_MICCAI/indexes/newE/numberGoodBad.npy",np.array([numberGood,numberBad]))
    below_th_q=[]
    below_th_t=[]
